...
def _addColorArtist(color_artists: dict, color, color_type_name: str, artist: Artist):
    """add a color use of an artist to the color index"""
    try:
        cmap = color.cmap
        value = color.value
    ...
    if mpl.colors.to_hex(color) in ["#000000", "#ffffff"]:
        return
    ...
    if cmap:
        if getattr(cmap, "get_color", None):
            for index, color in enumerate(cmap.get_color()):
                ...
                color_artists.setdefault(color, []).append(
                    [color_type_name, artist, value, cmap, index]
                )
        else:
            color_artists.setdefault(cmap, []).append(
...
def addArtistColors(color_artists: dict, artist: Artist):
    """find all colors used by an Artist and its editable children"""
    ...
    if not isinstance(artist, (mpl.text.Text, mpl.axis.XTick, mpl.axis.YTick)):
        for child in artist.get_children():
            addArtistColors(color_artists, child)
    ...
    for color_type_name in [
        ...
        for color in colors:
            _addColorArtist(color_artists, color, color_type_name, artist)
...
def addChildren(color_artists: dict, parent: Artist):
    ...
    for artist in parent.get_children():
        addArtistColors(color_artists, artist)


def artistListColors(artist: Artist) -> dict:
    """return color uses for an Artist and its editable children"""
    color_artists = {}
    addArtistColors(color_artists, artist)
    return color_artists


def figureListColors(figure: Figure):
    """add all artist with colors to a list in the figure"""
    figure.color_artists = artistListColors(figure)


def figureSwapColor(
    figure: Figure, new_color: str, color_base: str, color_artists: dict = None
):
    """swap two colors of a figure"""
    if color_artists is None:
        if getattr(figure, "color_artists", None) is None:
            figureListColors(figure)
        color_artists = figure.color_artists
    changed_cmaps = []
    maps = plt.colormaps()
    for data in color_artists[color_base]:
        # get the data
...
class ColorChooserWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent: QtWidgets, canvas: FigureCanvas, signals=None):
        ...
        if signals:
            ...
            signals.figure_element_selected.connect(self.setSelectedElement)
            signals.figure_selection_property_changed.connect(self.updateColorsText)

    def setCanvas(self, canvas):
        self.canvas = canvas
        self.selected_element = None

    def setSelectedElement(self, element: Artist):
        """set the artist whose colors should be shown in this panel"""
    ...
    def updateColorsText(self):
        ...
        source = self.selected_element or self.canvas.figure
        if source is self.canvas.figure:
            figureListColors(self.canvas.figure)
            self.color_artist_map = self.canvas.figure.color_artists
        else:
            self.color_artist_map = artistListColors(source)
        self.color_artists = list(self.color_artist_map)
    ...
    def color_selected(self, new_color: str, color_base: str):
        ...
        figureSwapColor(
            ...
            new_color,
            color_base,
            color_artists=self.color_artist_map,
        )
        # redraw the plot
        self.canvas.draw()
